...
notice 0x01 * BLOCK_SIZE makes the same string as 0x01 * (BLOCK_SIZE-1),
so the hash will be the same
"""
def pad(data):
...
def cryptohash(msg):
    initial_state = xor(Y_bytes, Z_bytes) # det
    msg_padded = pad(msg) # det
    msg_blocks = blocks(msg_padded) # det
    for i,b in enumerate(msg_blocks):
        mix_in = scramble_block(b) # inv: yes!
        for _ in range(i):
...
"""
https://www.researchgate.net/publication/221348188_Kleptography_Using_Cryptography_Against_Cryptography
"""
...
m = pad("hi".encode())
sc1 = scramble_block(m)
uc1 = unscramble_block(sc1)

assert(uc1 == m) # yes!!

twoblockmsg = ("a"*(BLOCK_SIZE+1))
twoblockmsg = pad(twoblockmsg.encode())

assert(len(twoblockmsg) == 2*len(m)) # yes

twoblockhash = bytes.fromhex(cryptohash(twoblockmsg))

initial_state = xor(Y_bytes, Z_bytes) # det
# for 1 block we skip lines 48 to 51, so
# cryptohash = state ^ scramble(m)
# => m = unscramble(cryptohash ^ state)

m = unscramble_block(xor(twoblockhash, initial_state))

assert(cryptohash(m) == twoblockhash.hex()) # YATTA!!!!!!!!!!
