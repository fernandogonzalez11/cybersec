...
# conic group law: https://www.sciencedirect.com/science/article/abs/pii/S0096300320304938
...
print(scalar_multiplication(G, n_a)==A) # TRUE!!!!!

shared_secret = gen_shared_secret(B, n_a)

def decrypt_flag(shared_secret: int, iv: bytes, flag: bytes):
    # Derive AES key from shared secret
    key = sha1(str(shared_secret).encode('ascii')).digest()[:16]
    # Decrypt flag
    cipher = AES.new(key, AES.MODE_CBC, iv)
